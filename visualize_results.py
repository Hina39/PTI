...
def plot_and_save_syn_images(syn_images, filenames): 
  for img, filename in zip(syn_images, filenames): 
      img = (img.permute(0, 2, 3, 1).squeeze() * 127.5 + 128).clamp(0, 255).to(torch.uint8).detach().cpu().numpy()
      plt.imshow(img)
      plt.axis('off') 
...
plot_and_save_syn_images([old_image, new_image], [f'./outputs/{image_name}_old.png', f'./outputs/{image_name}_new.png'])
...
for direction, factor_and_edit in latents_after_edit.items():
  ...
  for latent in factor_and_edit.values():
    ...
    plot_and_save_syn_images([old_image, new_image], [f'./outputs/{image_name}_old_edit.png', f'./outputs/{image_name}_new_edit.png'])
