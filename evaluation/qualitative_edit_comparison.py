...
class EditComparison:
    ...
    def save_reconstruction_images(
        self, image_latents, new_inv_image_latent, new_G, target_image
    ):
        if self.save_concatenated_images:
            save_concat_image(
                self.concat_base_dir,
                image_latents,
                new_inv_image_latent,
                new_G,
    ...
    def save_interfacegan_edits(
        ...
        new_w_inv_edits = self.latent_editor.get_single_interface_gan_edits(
            new_inv_image_latent, interfacegan_factors
        )

        inv_edits = []
        for latent in image_latents:
            inv_edits.append(
                self.latent_editor.get_single_interface_gan_edits(
                    latent, interfacegan_factors
                )
            )
        ...
        for direction, edits in new_w_inv_edits.items():
            for factor, edit_tensor in edits.items():
                if self.save_concatenated_images:
                    save_concat_image(
                        self.concat_base_dir,
                        [edits[direction][factor] for edits in inv_edits],
    ...
    def save_ganspace_edits(
        self, image_latents, new_inv_image_latent, factors, new_G, target_image
    ):
        new_w_inv_edits = self.latent_editor.get_single_ganspace_edits(
            new_inv_image_latent, factors
        )
        inv_edits = []
        for latent in image_latents:
            inv_edits.append(
                self.latent_editor.get_single_ganspace_edits(latent, factors)
            )
        ...
        for idx in range(len(new_w_inv_edits)):
            if self.save_concatenated_images:
                save_concat_image(
                    self.concat_base_dir,
                    [edit[idx] for edit in inv_edits],
                    new_w_inv_edits[idx],
                    new_G,
                    self.experiment_creator.old_G,
                    f"ganspace_{idx}",
            ...
            if self.save_single_images:
                save_single_image(
                    self.single_base_dir, new_w_inv_edits[idx], new_G, f"ganspace_{idx}"
                )

    def run_experiment(
        self, run_pt, create_other_latents, use_multi_id_training, use_wandb=False
    ):
        images_counter = 0
        new_G = None
        ...
        self.experiment_creator.run_experiment(
            run_pt, create_other_latents, use_multi_id_training, use_wandb
        )
        ...
        for idx, image_path in tqdm(
            ...
            image_latents, new_inv_image_latent = self.get_image_latent_codes(
                image_name
            )

            self.create_output_dirs(image_name)

            self.save_reconstruction_images(
                image_latents, new_inv_image_latent, new_G, target_image
            )
...
def run_pti_and_full_edit(iid):
    ...
    edit_figure_creator = EditComparison(
        save_single_images=True,
        save_concatenated_images=True,
        run_id=f"{paths_config.input_data_id}_pti_full_edit_{iid}",
    )
    edit_figure_creator.run_experiment(
        True, True, use_multi_id_training=False, use_wandb=False
    )
...
def pti_no_comparison(iid):
    ...
    edit_figure_creator.run_experiment(
        True, False, use_multi_id_training=False, use_wandb=False
    )
...
def edits_for_existed_experiment(run_id):
    evaluation_config.evaluated_methods = ["SG2Plus", "e4e", "SG2"]
    edit_figure_creator = EditComparison(
        save_single_images=True, save_concatenated_images=True, run_id=run_id
    )
    edit_figure_creator.run_experiment(
        False, True, use_multi_id_training=False, use_wandb=False
    )
