...
def SplitS(ds_p, M, if_std):
    ...
    for i in range(len(M.s_names)):
        if ("RGB" not in M.s_names[i]) and (not len(all_ds[tmp_index]) == 0):
...
if __name__ == "__main__":
    ...
    file_path = (
        "/cs/labs/danix/wuzongze/Tansformer_Manipulation/CLIP/results/"
        + M.dataset_name
        + "/"
    )
    fs3 = GetFs(file_path)
    ...
    M.num_images = 30
    dlatent_tmp = [
        tmp[M.img_index : (M.img_index + M.num_images)] for tmp in M.dlatents
    ]
    # %%
