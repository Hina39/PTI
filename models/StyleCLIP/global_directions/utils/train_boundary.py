...
def train_boundary(
    ...
    if chosen_num_or_ratio <= 0:
        raise ValueError(
            f"Input `chosen_num_or_ratio` should be positive, "
    ...
    train_label = np.concatenate(
        [np.ones(train_num, dtype=np.int), np.zeros(train_num, dtype=np.int)], axis=0
    )
    ...
    val_label = np.concatenate(
        [np.ones(val_num, dtype=np.int), np.zeros(val_num, dtype=np.int)], axis=0
    )
    ...
    print(
        f"  Remaining: {remaining_positive_num} positive, "
        f"{remaining_negative_num} negative."
    )
