...
def adjust_pixel_range(images, min_val=-1.0, max_val=1.0, channel_order="NCHW"):
    ...
    if images.ndim != 4:
        raise ValueError(
            f"Input images are expected to be with shape `NCHW` or "
            f"`NHWC`, but `{images.shape}` is received!"
        )
    if channel_order == "NCHW" and images.shape[1] not in [1, 3]:
        raise ValueError(
            "Input images should have 1 or 3 channels under `NCHW` " "channel order!"
        )
    if channel_order == "NHWC" and images.shape[3] not in [1, 3]:
        raise ValueError(
            "Input images should have 1 or 3 channels under `NHWC` " "channel order!"
        )
