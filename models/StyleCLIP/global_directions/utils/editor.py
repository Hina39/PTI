...
def manipulate(
    ...
    *code_shape], and `boundary` to be with shape [1, *code_shape] and unit norm.
    ...
    if isinstance(layerwise_manipulation_strength, (int, float)):
        ...
        if len(s) != num_layers:
            raise ValueError(
                f"Shape of layer-wise manipulation strength `{len(s)}` "
        ...
        if s.size != num_layers:
            raise ValueError(
                f"Shape of layer-wise manipulation strength `{s.size}` "
                f"mismatches number of layers `{num_layers}`!"
            )
    else:
        raise ValueError("Unsupported type of `layerwise_manipulation_strength`!")
...
def MPC(proj, x, mindex, start_distance, end_distance, step):
    ...
    x1 = proj.transform(
        x[:, 0, 0, :]
    )  # /np.sqrt(proj.explained_variance_) # (batch_size,num_pc)
...
def parse_boundary_list(boundary_list_path):
    ...
    if not os.path.isfile(boundary_list_path):
        raise ValueError("Boundary list `boundary_list_path` does not exist!")
