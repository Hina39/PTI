...
class PlayInteractively:  # Controller
    ...
    def addImage(self, img):
        self.view.bg.create_image(
            self.view.width / 2, self.view.height / 2, image=img, anchor="center"
        )
    ...
    def openfn(self):
        filename = askopenfilename(
            title="open",
            initialdir="./data/" + self.style_clip.M.dataset_name + "/",
            filetypes=[("all image format", ".jpg"), ("all image format", ".png")],
        )
        return filename
    ...
    def open_img(self, event):
        ...
        self.style_clip.M.dlatent_tmp = [
            tmp[img_index : (img_index + 1)] for tmp in self.style_clip.M.dlatents
        ]
