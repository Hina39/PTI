...
class StyleCLIP:
    ...
    def SetInitP(self):
        ...
        img_index = 0
        self.M.dlatent_tmp = [
            tmp[img_index : (img_index + 1)] for tmp in self.M.dlatents
        ]
    ...
    def GetDt2(self):
        ...
        for i in range(len(betas)):
            boundary_tmp2, num_c = GetBoundary(
                self.fs3, self.dt, self.M, threshold=betas[i]
            )
            print(betas[i])
            num_cs.append(num_c)
    ...
    def GetCode(self):
        boundary_tmp2, num_c = GetBoundary(
            self.fs3, self.dt, self.M, threshold=self.beta
        )
