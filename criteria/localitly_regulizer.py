...
class Space_Regulizer:
    ...
    def get_morphed_w_code(self, new_w_code, fixed_w):
        ...
        direction_to_move = (
            hyperparameters.regulizer_alpha
    ...
    def get_image_from_ws(self, w_codes, G):
        return torch.cat(
            [
                G.synthesis(w_code, noise_mode="none", force_fp32=True)
                for w_code in w_codes
            ]
        )

    def ball_holder_loss_lazy(
        ...
        territory_indicator_ws = [
            self.get_morphed_w_code(w_code.unsqueeze(0), w_batch)
        ...
        for w_code in territory_indicator_ws:
            ...
            if hyperparameters.regulizer_l2_lambda > 0:
                ...
                if use_wandb:
                    wandb.log(
                        {"space_regulizer_l2_loss_val": l2_loss_val.detach().cpu()},
